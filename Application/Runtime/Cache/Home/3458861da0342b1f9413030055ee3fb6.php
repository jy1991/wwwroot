...
<script src="/wwwroot/Public/cxselect/jquery.cxselect.js" type="text/javascript"></script>
<script type="text/javascript">
    $.cxSelect.defaults.url = 'select.json'; 
    $.cxSelect.defaults.nodata = 'none'; 
</script>
<link href="/wwwroot/Public/Dist/css/product.css" rel="stylesheet">

<!--商品详情-->
<div id="main_product">

  <div id="tsShopContainer">
  <div id="tsImgS"><a href="/wwwroot/Uploads/<?php echo ($pro['img'][0]['path']); echo ($pro['img'][0]['name']); ?>" title="Images" class="MagicZoom" id="MagicZoom"><img width="300" height="300" src="/wwwroot/Uploads/<?php echo ($pro['img'][0]['path']); echo ($pro['img'][0]['name']); ?>" /></a></div>
  <div id="tsPicContainer" style="margin-top:20px;">
    <div id="tsImgSArrL" onclick="tsScrollArrLeft()"></div>
    <div id="tsImgSCon">
      <ul>
        <?php if(is_array($pro["img"])): $i = 0; $__LIST__ = $pro["img"];if( count($__LIST__)==0 ) : echo "没有图片" ;else: foreach($__LIST__ as $key=>$img): $mod = ($i % 2 );++$i;?><li onclick="showPic(<?php echo ($i-1); ?>)" rel="MagicZoom" ><img height="42" width="42" src="/wwwroot/Uploads/<?php echo ($img["path"]); echo ($img["name"]); ?>" tsImgS="/wwwroot/Uploads/<?php echo ($img["path"]); echo ($img["name"]); ?>" /></li><?php endforeach; endif; else: echo "没有图片" ;endif; ?> 
      </ul>
    </div>
...
<script src="/wwwroot/Public/show/js/ShopShow.js"></script>
  
    <div id="element_id" class="row">
            <div class="col-xs-10">
                <form class="form-horizontal" action='<?php echo U('Cart/add');?>' method='post'>
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">商品名</label>
                    <div class="col-sm-9">
                      <input type="text" class="form-control"  name="" value="<?php echo ($pro["name"]); ?>" disabled>
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">特性</label>
                    <div class="col-sm-9">
                      <input type="text" class="form-control"  name="" value="<?php echo ($pro["ad"]); ?>" disabled style="color:red">
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">华为价</label>
                    <div class="col-sm-9">
                      <input type="text" class="form-control" name="price" value="<?php echo ($pro["price"]); ?>" disabled style="color:red; font-size:18px;">
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">商品编码</label>
                    <div class="col-sm-9">
                      <input type="text" class="form-control" value="<?php echo ($pro["number"]); ?>" disabled >
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">商品评分</label>
                    <div class="col-sm-9">
                      <input type="text" class="form-control"  name="" value="<?php echo ($pro["point"]); ?> ( 共 <?php echo ($pro["comment"]); ?> 条评论 )" disabled >
                    </div>
                  </div>
                  <hr>
<?php if(is_array($attr)): $i = 0; $__LIST__ = $attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$select): $mod = ($i % 2 );++$i;?><div class="form-group">
        <label for="" class="col-sm-3 control-label">选择<?php echo ($select["name"]); ?></label>
        <div class="col-sm-9">
           <select class="form-control" name='<?php echo ($select["title"]); ?>' data-value="请选择">
            
             <?php if(is_array($select["value"])): $i = 0; $__LIST__ = $select["value"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><option value="<?php echo ($value["id"]); ?>"><?php echo ($value["value"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">
    ...
    selects: [<?php echo ($title); ?>],  // 数组格式，请注意顺序 
    nodata: 'none' 
}); 
</script>                 
                 <div class="form-group">
                    ...
                    <div class="col-sm-9">
                      <input type="number" class="form-control"  name="num" value="1" min="1" max="<?php echo ($pro["stock"]); ?>">
                    </div>
                  </div>

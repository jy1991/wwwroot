...
class AdminController extends Controller {
	...
	public function _initialize(){
		...
		//权限过滤
		$controller_name = CONTROLLER_NAME; //获取控制器名
		$do_name = ACTION_NAME; //获取方法名
        $worklist = $_SESSION['admin_user']['worklist']; //获取权限列表

		//让超级管理员admin拥有所有权限
		if($_SESSION['admin_user']['admin_user'] != 'admin'){
			//验证操作权限
			if(empty($worklist[$controller_name]) || !in_array($do_name,$worklist[$controller_name])){
				$this->error("您没有操作权限！");
				exit;
			}
